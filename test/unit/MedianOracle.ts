...
import { BigNumber, Contract, ContractFactory, Signer, BigNumberish } from 'ethers'
...
async function setupContractsAndAccounts() {
  ...
  oracle = await upgrades.deployProxy(
    (await ethers.getContractFactory('MedianOracle')).connect(deployer),
    [60, 10, 1],
    {
      initializer: 'initialize(uint256,uint256,uint256)',
    },
  )
}
...
describe('MedianOracle:initializer', async function () {
  const deploy_ = async (p1: number, p2: number, p3: number) => {
  ...
  it('should fail if a parameter is invalid', async function () {
    await expect(deploy_(60, 10, 0)).to.be.reverted
    await expect(deploy_(60 * 60 * 24 * 365 * 11, 10, 1)).to.be.reverted
    await expect(oracle.setReportExpirationTimeSec(60 * 60 * 24 * 365 * 11)).to
      .be.reverted
  })
  it('shouldnt fail with correct parameteres', async function () {
    await expect(deploy_(0, 0, 1)).not.to.be.reverted
  })
})
...
describe('MedianOracle:addProvider:accessControl', async function () {
  ...
  it('should NOT be callable by non-owner', async function () {
    await expect(oracle.connect(B).addProvider(await A.getAddress())).to.be.reverted
  })
})
